...
def xcode_caveats(ac_download_url); <<~EOS
  ...
  environment variable 'HOMEBREW_XCODE_DOWNLOAD_PATH'.  This can be 
  specified as a local path (using the file:// scheme) or other URL, 
  and will be used in the url for this cask. It defaults to 
  ${HOME}/Downloads
...
def xcode_xip_caveats(ac_download_url, appname)
  ...
  following commands:
    $ open #{ac_download_url.split("/")[-1].gsub(/\.dmg$/, '.xip')} && \\
      ...
      rm -rf Xcode-tmp && \\
      shasum -a 256 #{ac_download_url.split("/")[-1]}
